...
class Crm(Tools):
    ...
    def __init__(self, species, bg, reactions, path='.', 
        ...
        def append_reaction(reaction, scale=1):
            ...
            elif header.split()[0].upper() == 'MCCCDB' and ('$' not in header):
#                coeffs=[[1e-99,1e-99]]
                coeffs=[[0.0, 0.0]]
                    ...
                    if '$' in rea:
                        ...
                        if '&' in rea:
                            ...
                            if header.split()[1].upper() == 'RELAXATION':
                                ...
                                coeffs=[[0.0, 0.0]]
                                sourcefile_n=reaction[0]
                                for y in range(isn, 1 + isv*self.vmax + isn*self.nmax):
                                    # Limit transitions to [0,vmax]
                                    if x + y in range(self.vmax + 1):
                                            coeffs=[[0.0, 0.0]]
                                            sourcefile=sourcefile_n.replace('$',str(x)).replace('&',str(x+y))
                                            with open(sourcefile, 'r') as f:
                                                for l in f:
                                                    l = l.strip()
                                                    if l[0] == '#':
                                            ...
                                            coeffs[:,-1] *= (5.29177210903e-9)**2 # Convert to CS to SI units
                                            coeffs[coeffs == 0] = 1e-99
                                            create_reaction(self.XY2num(header,x,y+x), [self.XY2num(rea,x,x+y)], coeffs, scale, sourcefile)
                        ...
                        # Independent upper state
                            ...
                            if header.split()[0].upper() == 'MCCCDB':
                                coeffs=[[0.0, 0.0]]
                                sourcefile_n=reaction[0]
                                for y in range(-1*('&' in rea), 2, 2):
                                    # Limit transitions to [0,vmax]
                                    if x + y in range(self.vmax + 1):
                                            coeffs=[[0.0, 0.0]]
                                            sourcefile=sourcefile_n.replace('$',str(x))
                                            with open(sourcefile, 'r') as f:
                                                for l in f:
                                                    l = l.strip()
                                                    if l[0] == '#':
                                            ...
                                            coeffs[:,-1] *= (5.29177210903e-9)**2 # Convert to CS to SI units
                                            coeffs[coeffs == 0] = 1e-99
                                            create_reaction(self.XY2num(header,x), [self.XY2num(rea,x)], coeffs, scale, sourcefile)
                                else:
