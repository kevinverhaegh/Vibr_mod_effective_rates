...
def extract_table(el_state,vibr_num):
    a0 = 5.29177210903e-11
    with open(f'rates/MCCC/X1Sg-excitation/vi={vibr_num}/MCCC-el-D2-'+el_state+f'_DE.X1Sg_vi={vibr_num}.txt','r') as file:
        table = []
...
def numpy_to_string(i): 
    part_1 = "\subsection{\n" +\
                f"Reaction 2.{i}l1\n"+\
                f"$ e + H_2(v={i}) \\rightarrow e + 2*H $ (dissociation)\n" +\
                "}\n"+\
...
states = ['a3Sg', 'b3Su', 'B1Su', 'Bp1Su', 'C1Pu', 'c3Pu', 'D1Pu', 'd3Pu', 'e3Su', 'EF1Sg', 'g3Sg', 'GK1Sg', 'H1Sg', 'h3Sg', 'I1Pg', 'i3Pg', 'J1Dg', 'j3Dg']

full_string = ''
for i in range(15):
    rate = np.zeros(len(Tev))

    # Summing the rates over all electronic states
    for state in states:    
        table = extract_table(state,i)
        fit = fit_temp(table,Tev)
        rate += eval_1D(fit, Tev)

    rate_fit = np.flip(np.polyfit(np.log(Tev),np.log(rate/1e-6),8))

    str1,str2=numpy_to_string(i)
    full_string += str1+block_string(rate_fit)+str2
